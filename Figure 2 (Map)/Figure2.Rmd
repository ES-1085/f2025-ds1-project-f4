...
p <- ggplot(world_data) +
  geom_sf(aes(fill = gdp_per_capita), color = "gray30", size = 0.2) +
...
ggplot(data = world_data) +
  geom_sf(aes(fill = v2x_libdem), color = "gray30", size = 0.2) +
  scale_fill_viridis_c(na.value = "lightgray",
...
```
``` {r Liberal_democracy_across_time}
world <- ne_countries(scale = "medium", returnclass = "sf")
world_data <- world %>%
  left_join(df, by = c("iso_a3" = "iso3c"))

df <- df %>% filter(year >= 2020 & year <= 2024)
...
animate(p,
  ...
  fps = 10,
  width = 900, height = 600,
  renderer = gifski_renderer("world_libdem_rating_2020_2024.gif")
)
